...
## Can a single resting-state brain scan predict how well someone thinks — across four psychiatric diagnoses?

This project applies **Connectome-Based Predictive Modelling (CPM)** (Shen et al., 2017) to test whether resting-state functional connectivity (rsFC) can transdiagnostically predict individual cognitive performance across schizophrenia, bipolar disorder, ADHD, and healthy controls — without using any diagnostic labels.

**Quick links:** [Background](#background) | [Data](#data) | [Methods](#methods) | [Results](#results) | [How to Reproduce](#how-to-reproduce) | [Limitations](#limitations--future-directions) | [References](#references)

---

## Background

Schizophrenia, bipolar disorder, and ADHD share overlapping cognitive impairment profiles across working memory, verbal memory, attention, processing speed, and inhibitory control. They differ in severity rather than which domains are affected (Vöhringer et al., 2013). Despite this, no neuroimaging biomarkers have been adopted in clinical psychiatric practice (Abi-Dargham et al., 2023).

Individual resting-state connectomes are stable, unique, and predictive of behaviour (Finn et al., 2015). If shared cognitive deficits across these disorders arise from common disruptions in functional brain organisation, then a single rsFC-based model should predict cognitive performance across all four groups. This is the **transdiagnostic hypothesis** tested here.
...
**Sample:** N = 260 (ADHD = 40, Bipolar = 49, Controls = 121, Schizophrenia = 50)
...
For each subject, Pearson correlation was computed between every pair of 82 regional timeseries to produce an 82x82 FC matrix. Fisher-Z transform was applied to normalise r-values. The upper triangle was flattened into a 3,321-edge feature vector per subject.

### Covariate Correction
Cognitive scores were residualised against age and gender before CPM. The diagnostic groups differ systematically in age and gender composition. Without correction, CPM picks up on demographic differences rather than genuine FC-cognition signal — working memory and attention showed spuriously significant predictions before correction (r approx. -0.19, p = 0.003) that disappeared after, confirming they were demographic confounds.
...
3. Select edges at p < 0.01 — positive network and negative network separately
4. Net strength = sum of positive edges minus sum of negative edges
5. Fit linear model: score ~ net strength (training only)
6. Predict held-out participant's score
7. Repeat for all N; accuracy = Pearson r (predicted vs observed)
...
![CPM Prediction Accuracy](CPM_bar_plot.png)

![Predicted vs Observed Scores](CPM_scatter_plots.png)

### What this means

The main finding is that a single resting-state connectome model, trained with no knowledge of diagnosis, can significantly predict verbal memory and inhibitory control across schizophrenia, bipolar disorder, ADHD, and healthy controls. This supports the transdiagnostic hypothesis: these cognitive deficits share a common neural substrate in functional brain organisation, not just a shared diagnostic label.

**Verbal memory (r = 0.149, p = 0.017)**

rsFC significantly predicts CVLT scores across all four groups. The CVLT measures the ability to learn and recall a word list across multiple trials, which is a core measure of episodic verbal memory. The fact that a single FC pattern predicts this across SZ, BD, ADHD, and controls suggests there is a common disruption in hippocampal-cortical connectivity underlying verbal recall deficits transdiagnostically, consistent with posterior hippocampus and default mode network alterations linked to episodic memory in psychiatric populations (Dugre et al., 2021).

**Inhibitory control (r = 0.139, p = 0.028)**

rsFC significantly predicts stop-signal reaction time (SSRT) across all four groups. SSRT measures how quickly a person can stop a prepotent motor response, so higher SSRT means worse inhibitory control. The positive r means the model correctly predicts worse inhibition for participants who actually performed worse. This is consistent with reduced right inferior frontal gyrus functional connectivity in SZ and BD during stop-signal tasks (Tsujii et al., 2018), and converges with and extends Lv et al. (2025) to include ADHD.

**Why only 2 out of 5 domains?**

This is itself an important finding. Working memory and attention were significant before covariate correction but lost significance after, meaning their apparent FC-cognition link was a demographic confound. Processing speed was non-significant before and after correction, suggesting resting-state FC may not be the right tool here; task-evoked FC during a speed task would be more appropriate.

**What this means clinically**

The domain-specificity of these results means a single resting-state scan cannot replace a full neuropsychological battery. However, the significant predictions for verbal memory and inhibitory control suggest rsFC could potentially flag at-risk individuals in these specific domains, which is relevant for early intervention in psychiatric populations where full neuropsychological assessments are resource-intensive.
...
├── UCLA_CPM_analysis.html         # Rendered output — view results without running code
├── CPM_summary.csv                # CPM prediction accuracy per cognitive domain
├── CPM_bar_plot.png               # Prediction accuracy bar chart
├── CPM_scatter_plots.png          # Predicted vs observed scatter plots
└── data/
...
2. Download `UCLA_CNP_fMRI_TS.mat` from Zenodo: https://doi.org/10.5281/zenodo.10431855 and place it at `data/UCLA_CNP_fMRI_TS.mat`

3. The behavioural and demographic data under `data/phenotype/` is already included in this repository, so no separate download is needed for that.

4. Open `UCLA_CPM_analysis.Rmd` in RStudio, set the working directory to the repo root, and knit. CPM takes about 10-15 minutes.

The `.html` output is also included so you can view all results and figures without running the code at all.
...
- **Atlas resolution:** 82 regions (Desikan-Killiany) — finer parcellations may improve prediction.
- **Permutation testing:** p-values from Pearson r should be validated with permutation-based null distributions for more robust statistical validation.
- **Network specificity:** Future work should identify which specific brain networks drive the verbal memory and inhibitory control predictions.
